...
def send_email(region):
    ...
    if region == "KR":
        ...
        while not Sent:
            try:    
                names, emails = get_contacts('receipients.txt') # read contacts
                ...
                filename_infra = glob.glob(os.path.join(pdf_path, 'Infra_in_detail_%s*.pdf' %(region)))[-1]
                filename_jennifer = glob.glob(os.path.join(pdf_path, 'Jennifer_dashboard_*.pdf'))[-1]
        ...
        while not Sent:
            try:    
                ...
                names, emails = get_contacts('receipients.txt') # read contacts
                message_template = read_template('message_%s.txt'%(region))
                ...
                filename_infra = glob.glob(os.path.join(pdf_path, 'Infra_in_detail_%s_*.pdf' %(region)))[-1]
